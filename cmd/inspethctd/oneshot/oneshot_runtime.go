...
func (session *oneshotSession) run(ctx context.Context, mode oneshotExecutionMode) error {
	...
	_ = debugHooks.Register(stepHook)
	_ = debugHooks.Register(newCallHook(session, engine.HookTypeExternalCall, "oneshot-call"))
	_ = debugHooks.Register(newCallHook(session, engine.HookTypeDelegateCall, "oneshot-delegate"))
	_ = debugHooks.Register(newCallHook(session, engine.HookTypeStaticCall, "oneshot-static"))
	_ = debugHooks.Register(newCallHook(session, engine.HookTypeCallCode, "oneshot-callcode"))
	_ = debugHooks.Register(newStorageHook(session, engine.HookTypeStorageRead, engine.HookTypeTransientLoad, "oneshot-storage-read", false))
	_ = debugHooks.Register(newStorageHook(session, engine.HookTypeStorageWrite, engine.HookTypeTransientStore, "oneshot-storage-write", true))
	_ = debugHooks.Register(newMemoryHook(session, engine.HookTypeMemoryRead, "oneshot-memory-read", false))
	_ = debugHooks.Register(newMemoryHook(session, engine.HookTypeMemoryWrite, "oneshot-memory-write", true))
	prepared.Config.Hooks = mergeMainHookRegistries(prepared.Config.Hooks, debugHooks)
